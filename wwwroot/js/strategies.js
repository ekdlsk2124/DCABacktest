...
// =============== 라오어 v2 ===============
// 차이점: 익절을 2단계로 분할(평단+5%에 50% 매도, +10%에 나머지 50% 매도), 최소 0.5 회차 규칙은 유지
    ...
    for (const c of ranged) {
        ...
        if (shares > 0) {
            ...
            if (c.close >= ac * (1 + tp1) && c.close < ac * (1 + tp2)) {
                ...
                investedLive *= 0.5; // 남은 절반의 원가만 유지
                shares -= sellQty;
...
// =============== 라오어 v2.1 ===============
// 차이점: splits의 50% 소진 이후에는 "평단 아래일 때만" 매수(최소 0.5 규칙 해제)
//        분할매도는 v2와 동일(5%/10%)
export function runRaoorV21(candles, unitPerRound, opt = {}) {
    ...
    for (const c of ranged) {
        ...
        // 보수화 전: 0.5 강제 + 평단 아래 0.5 추가
        // 보수화 후: 평단 아래일 때만 0.5씩(최대 1회차/일)
        if (!conservative) {
            if (canBuy(0.5)) {
                const amt = unitPerRound * 0.5, qty = amt / c.close;
                shares += qty; investedLive += amt; investedCum += amt; usedRounds += 0.5;
        ...
        if (shares > 0) {
            ...
            if (c.close >= ac * (1 + tp1) && c.close < ac * (1 + tp2)) {
                ...
                cash += shares * c.close;
                shares = 0; investedLive = 0; usedRounds = 0; // 리셋
            }
        }
...
// =============== 라오어 v3 ===============
// 아이디어: v2.1 규율 + 익절 후 다음 날 즉시 재진입을 가정(사이클 자동 반복)
    ...
    for (let i = 0; i < ranged.length; i++) {
        ...
        if (shares > 0) {
            ...
            if (c.close >= ac * (1 + tp1) && c.close < ac * (1 + tp2)) {
                ...
                cash += shares * c.close;
                shares = 0; investedLive = 0; usedRounds = 0; // 사이클 종료
                // v3: 다음 캔들(다음 날)부터 새 사이클 자동 시작 (로직상 자연히 진행)
            }
        }
